...
> **Hermes agent note:** This skill lives at `~/.hermes/skills/creative/zenmux-image-generation/`.
> All `skills/zenmux-image-generation/` paths below resolve relative to that root.
> Use `~/.hermes/skills/creative/zenmux-image-generation/` when calling scripts from terminal().
>
> **Protocol warning:** ZenMux's `gpt-image-2` defaults to Vertex AI protocol, NOT OpenAI
> Images protocol. Calling `/api/v1/images/generations` directly returns 403. Use
> `scripts/generate_gemini.py` (Vertex AI) for gpt-image-2, or
> `scripts/generate_openai.py` only when the user explicitly requests OpenAI Images protocol.
> See `references/hermes-setup.md` for the full diagnostic session.
...
- **Model not found / not authorized** → re-run `list_models.sh` to confirm the
  model id is current.
- **403 on generate call but model lists fine** → Protocol mismatch. The model
  is listed via Vertex AI endpoint but may not be callable via OpenAI Images
  protocol. Always prefer `scripts/generate_gemini.py` for gpt-image-2 unless
  the user explicitly asked for OpenAI Images protocol. If both protocols 403,
  the API key lacks execution permissions — user must check ZenMux dashboard.
  Full diagnostic: see `references/hermes-setup.md`.
...
- **Reference image too large / wrong type** → suggest converting to PNG/JPEG
  under 7 MB.
- **`generate_openai.py` hangs or times out (180s+ no output)** → the `uv run`
  path through the skill scripts can hang on first use (venv creation) or on
  large/high-quality images. Fall back to a direct inline Python call using the
  `openai` package and `ZENMUX_API_KEY` from the environment:

  ```python
  import os, base64
  from openai import OpenAI
  client = OpenAI(base_url="https://zenmux.ai/api/v1", api_key=os.environ["ZENMUX_API_KEY"])
  img = client.images.generate(model="gpt-image-2", prompt="...", n=1, size="1792x1024")
  image_bytes = base64.b64decode(img.data[0].b64_json)
  with open("output.png", "wb") as f: f.write(image_bytes)
  ```

  This approach uses `client.images.generate()` (OpenAI Images protocol) directly
  and bypasses the `uv` managed environment. It has been verified to work with
  `gpt-image-2` at `quality=high` and `size=1792x1024` (~2 min per image).
  For batch generation, wrap in a loop with `time.sleep(1)` between calls.
  Install the `openai` package with `pip3 install openai` if needed.
