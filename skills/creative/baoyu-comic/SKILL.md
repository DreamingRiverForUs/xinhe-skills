...
Trigger this skill when the user asks to create a knowledge/educational comic, biography comic, tutorial comic, or uses terms like "知识漫画", "教育漫画", "绘本", or "Logicomix-style". The user provides content (text, file path, URL, or topic) and optionally specifies art style, tone, layout, aspect ratio, or language.

**用户风格偏好**：默认使用 `minimalist`（极简手绘）或 `chalk`（粉笔）风格，不要 `ohmsha`（日式漫画）。角色用火柴人或极简线条，黑墨+暖奶油纸底，稀疏暖色点缀。竖版 1024x1536。中文手写对白。文章配图场景：扫描故事中的高画面感时刻（对话、情绪转折、具体动作），选 2-3 个并行生成漫画嵌入。
