...
- **TTS:** `npx hyperframes tts "Script text" --voice af_nova --output narration.wav`. List voices with `--list`. Voice ID first letter encodes language (`a`/`b`=English, `e`=Spanish, `f`=French, `j`=Japanese, `z`=Mandarin, etc.) — the CLI auto-infers the phonemizer locale; pass `--lang` only to override. Non-English phonemization requires `espeak-ng` installed system-wide.
  - **Chinese TTS fallback:** HyperFrames' built-in kokoro-onnx requires `espeak-ng` for Chinese phonemization (often not installed). For Chinese narration, prefer Microsoft Edge-TTS which has excellent Mandarin voices and requires no extra system deps:
    ```bash
    pip3 install edge-tts
    python3 -m edge_tts --voice zh-CN-XiaoxiaoNeural --rate=-5% \
      --text "旁白文字" --write-media output.wav
    ```
    `zh-CN-XiaoxiaoNeural` (female, warm) is ideal for report-style narration.
    `zh-CN-YunyangNeural` (male, professional) for authoritative tones.
    Generate each scene's audio separately, then use `ffprobe` to get durations for timeline alignment.
...
- **System Chrome (not `chrome-headless-shell`) on older HyperFrames** — pre-0.4.2 renders hang for 120s then timeout. On 0.4.2+, auto-detect falls back to screenshot mode and system Chrome works fine (verified on macOS 0.6.2: 8.6s render). `hyperframes doctor` reports which binary will be used. If `npx puppeteer browsers install chrome-headless-shell` fails with corrupted cache, system Chrome is a working fallback — no need to force it.
...
- **`--example warm-grain` (and other non-blank templates) fetch from a remote registry** — fails with "registry unreachable or empty" when offline. Use `--example blank` for offline scaffolding and build from scratch.
- **Lint `gsap_exit_missing_hard_kill` warning** — every exit tween (e.g., `tl.to("#scene", { opacity: 0 }, 3.5)`) that ends at a scene boundary must be followed by `tl.set("#scene", { opacity: 0 }, <boundary_time>)` to prevent stale visibility state during non-linear seeking. Not doing this leaves ghost elements visible when seeking.
- **Using video for audio** — always muted `<video>` + separate `<audio>`.
- **Floating-point clip boundary overlap** — when adjacent clips have mathematically equal end/start times (e.g. `data-duration="7.1"` ending at 53.8 and next `data-start="53.8"`), the linter reports `overlapping_clips_same_track` due to floating-point representation (53.800000000000004 > 53.8). **Fix:** add a 0.01s gap between every adjacent clip boundary. Round down the previous clip's duration by 0.01s, keeping the next clip's start at the integer boundary. Example: `data-duration="7.09"` ending at 53.79, next `data-start="53.8"`. Apply to all clip tracks consistently (div, img, audio).
...
- [screenshot-tutorial-workflow.md](references/screenshot-tutorial-workflow.md) — screenshot+TTS+HyperFrames pipeline for tutorial/demo videos (5 phases: capture → script → timeline → compose → render)
- [troubleshooting.md](references/troubleshooting.md) — OpenClaw fix, env vars, common render errors
- [multi-scene-lint-fixes.md](references/multi-scene-lint-fixes.md) — overlapping tracks, missing clip class, visibility kill, audio overlap, crossfade pattern
